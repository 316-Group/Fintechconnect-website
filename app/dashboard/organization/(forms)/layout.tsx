...
const ONBOARDING_STEPS = [
  { id: 1, title: "Organization", href: "/dashboard/organization/businessidentity" },
  { id: 2, title: "Compliance", href: "/dashboard/organization/regulatorycompliance" },
  { id: 3, title: "Beneficial Ownership", href: "/dashboard/organization/beneficial-ownership" },
  { id: 4, title: "Documents", href: "/dashboard/organization/documents" },
  { id: 5, title: "Verification", href: "/dashboard/organization/verification" },
  { id: 6, title: "Review", href: "/dashboard/organization/review" },
];
...
export default function OrganizationLayout({
  ...
  // Find active step based on exact or matching path
  const currentStepIndex =
    ONBOARDING_STEPS.findIndex(
      (step) => pathname === step.href || pathname.includes(step.href)
    ) + 1 || 1;

  const progressPercentage = Math.round(
    (currentStepIndex / ONBOARDING_STEPS.length) * 100
  );

  return (
    <div className="dashboard-scope min-h-screen bg-[#F8FAFC] flex flex-col font-sans text-slate-800">
      {/* Top Navigation Bar */}
      <header className="h-16 bg-white border-b border-slate-200 px-4 sm:px-8 flex items-center justify-between sticky top-0 z-30">
        <div className="flex items-center gap-6 lg:gap-10">
          {/* Logo */}
          <Link href="/dashboard/organization" className="flex items-center gap-2 shrink-0">
            <svg width="120" height="40" viewBox="100 0 150 201" fill="none" xmlns="http://www.w3.org/2000/svg" className="sm:w-[150px] sm:h-[50px]">
        ...
        <div className="flex items-center gap-2 sm:gap-4 text-slate-500">
          <button className="p-1.5 hover:bg-slate-100 rounded-lg transition" aria-label="Notifications">
            <Bell className="w-4 h-4" />
          </button>
          <button className="p-1.5 hover:bg-slate-100 rounded-lg transition" aria-label="Help">
          ...
          <button
            ...
            className="p-1.5 hover:bg-slate-100 rounded-lg transition lg:hidden text-slate-700 ml-1"
      ...
      {isMobileNavOpen && (
        <div className="lg:hidden bg-white border-b border-slate-200 px-4 py-3 space-y-1 z-20 sticky top-16 shadow-lg">
          {NAV_ITEMS.map((item) => {
            const isDashboard =
              item.label.toLowerCase() === "dashboard" ||
              item.href === "/" ||
      ...
      {/* Main Container */}
      <div className="flex-1 max-w-full w-full mx-auto px-4 sm:px-8 md:px-12 lg:px-20 py-4 sm:py-8 flex flex-col lg:flex-row gap-6 lg:gap-12">
        {/* Mobile Stepper Accordion Card */}
        <div className="block lg:hidden bg-white p-4 rounded-xl border border-slate-200 shadow-sm space-y-3">
          <div className="flex items-center justify-between">
            <div>
              <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block">
                Current Step ({currentStepIndex}/{ONBOARDING_STEPS.length})
              </span>
              <h3 className="text-sm font-bold text-slate-900">
                {ONBOARDING_STEPS[currentStepIndex - 1]?.title}
              </h3>
            </div>
            <button
              onClick={() => setIsMobileStepperOpen(!isMobileStepperOpen)}
              className="text-xs font-semibold text-blue-600 flex items-center gap-1 hover:underline p-1"
            >
              {isMobileStepperOpen ? "Hide Steps" : "View Steps"}
              <ChevronDown
                className={`w-4 h-4 transition-transform duration-200 ${
                  isMobileStepperOpen ? "rotate-180" : ""
                }`}
              />
            </button>
          </div>
          ...
          <button
            type="button"
            className="w-full bg-[#18181B] hover:bg-black text-white text-xs font-semibold py-2.5 rounded-md transition shadow-sm mt-2"
          >
            Save Progress
          </button>
        </div>
        ...
        {/* Desktop Sidebar */}
        <aside className="hidden lg:flex w-56 shrink-0 flex-col select-none">
          <h2 className="text-[11px] font-bold tracking-wider text-slate-400 uppercase mb-6">
            Complete Onboarding
          </h2>
          ...
          <div className="space-y-3">
            <button
              type="button"
              className="w-full bg-[#18181B] hover:bg-black text-white text-xs font-semibold py-2.5 rounded-md transition shadow-sm"
            >
              Save Progress
            </button>

            <div className="bg-slate-200/50 rounded-md p-3 border border-slate-200/50">
              <span className="text-[9px] font-bold uppercase tracking-wider text-slate-400 block mb-1">
                Current Status
              </span>
              <div className="flex items-center justify-between text-xs font-bold text-slate-800 mb-2">
                <span>Step {currentStepIndex} of {ONBOARDING_STEPS.length}</span>
        ...
        <main className="flex-1 min-w-0">
          <div className="w-full bg-white py-4 sm:py-6 px-4 sm:px-8 rounded-xl sm:rounded-none border sm:border-none border-slate-200 shadow-sm sm:shadow-none">
            {children}
          </div>
        </main>
      </div>
