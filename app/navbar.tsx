  ...
  <div className="absolute top-full left-1/3 -translate-x-2/5 mt-2 w-[95vw] max-w-[600px] bg-white rounded-2xl shadow-2xl overflow-hidden z-50">
    ...
    <div className="p-6 space-y-3">
      {navItems[menu as keyof typeof navItems]?.map((item) => (
        ...
        {/* 1. Backdrop Overlay (Darkens the remaining 1/3 of the screen) */}
        {mobileMenuOpen && (
          <div 
        ...
        <div className={`fixed top-0 left-0 bottom-0 w-3/3 sm:w-1/2 bg-black border-r border-slate-800 z-[100] transform transition-transform duration-300 ease-in-out ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:hidden overflow-y-auto`}>
          ...
          <div className="flex flex-col p-6 gap-4">
            {Object.keys(navItems).map((menu) => {
              ...
              // Determine which dataset maps to this item
              ...
              return (
                <div key={menu} className="border-b border-slate-900 pb-3 last:border-none">
                  {/* Menu Item Header Clickable Toggle */}
                  <button 
                    onClick={() => {
      ...
      <div key={subCategory} className="mt-1">
        ...
        <div className="pl-2 flex flex-col gap-2">
          {activeModules[subCategory].map((module, idx) => (
            ...
            {/* Action CTA Button */}
            <button className="mt-6 bg-blue-600 hover:bg-blue-700 text-white px-5 py-3 rounded-full font-medium w-full text-sm transition-colors text-center">
              Book a Demo
            </button>
          </div>
        </div>
