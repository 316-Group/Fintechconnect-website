...
export default function VerifyEmailPage() {
  ...
  const handleInputChange = (index: number, value: string) => {
    ...
    const newOtp = [...otp];
    // Keep only the last typed character if replacing
  ...
  return (
    <div className="min-h-screen w-full bg-[#f4f7ff] flex flex-col justify-between items-center relative font-sans overflow-x-hidden">
      {/* Background diagonal split gradient effect */}
      <div 
        className="absolute inset-0 z-0 bg-white"
        style={{ clipPath: "polygon(0 35%, 100% 58%, 100% 100%, 0 100%)" }}
      />
      <div 
        className="absolute inset-0 z-0 bg-[#ebf1fd]"
        style={{ clipPath: "polygon(0 0, 100% 0, 100% 58%, 0 35%)" }}
      />

      {/* Main Content Area */}
      <div className="z-10 w-full max-w-md px-4 pt-10 pb-8 flex flex-col items-center my-auto">
        
        {/* Centered Brand Logo (Hidden when verified to match target design) */}
        {!isVerified && (
        ...
        {/* Card Container */}
        <div className="w-full bg-white rounded-2xl shadow-xl shadow-blue-950/5 p-8 sm:p-12 border border-slate-100/80">
          {!isVerified ? (
            /* OTP Input Form */
            <>
              ...
              {/* Error Banner */}
              {error && (
                <div className="mt-4 p-3 rounded-xl bg-red-50 border border-red-200 text-red-600 text-xs text-center font-medium">
                  {error}
                </div>
              )}
              ...
              <form onSubmit={handleVerify} className="mt-6 space-y-6">
                ...
                <div className="flex justify-between items-center gap-2">
                  {otp.map((digit, index) => (
                    <input
                      ...
                      onPaste={handlePaste}
                      placeholder="·"
                      className={`w-11 h-12 text-center text-lg font-bold rounded-xl bg-white border transition-all placeholder-slate-300 focus:outline-none ${
                        digit || index === otp.findIndex((val) => val === "")
                ...
                <div>
                  <button
                    type="submit"
                    disabled={isLoading}
                    className="w-full py-3.5 px-4 rounded-xl bg-[#2f65ff] hover:bg-blue-700 text-xs font-semibold text-white uppercase tracking-wider shadow-lg shadow-blue-500/20 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                  >
              ...
              <div className="mt-4 bg-[#f4f7ff] rounded-xl py-3 px-4 flex items-center justify-center text-xs text-slate-500 border border-slate-100/60">
                ...
                <button
                  ...
                  className="ml-1 text-[#2f65ff] font-bold hover:underline focus:outline-none"
                >
            ...
            /* Verified Success Design */
            <div className="h-[350px] flex flex-col items-center text-center py-2">
              {/* Checkmark Badge */}
              <div className="w-14 h-14 rounded-full bg-[#ebf3ff] flex items-center justify-center mb-6">
              ...
              <p className="text-sm text-slate-500 leading-relaxed max-w-xl mb-8">
                Your account is now active. Let's get started with your organization setup to unlock the full potential of Fintech Connect.
              </p>
              ...
              <button
                ...
                className="w-full py-3.5 px-4 rounded-lg bg-[#0052cc] hover:bg-blue-700 text-xs font-semibold text-white tracking-wide shadow-md transition-all flex items-center justify-center gap-2"
              >
        ...
        {/* Terms & Privacy text below card */}
        <p className="mt-8 text-center text-xs text-slate-500 font-normal max-w-sm leading-relaxed">
          By signing up, you agree to our{" "}
          <Link href="/terms" className="text-[#0052cc] font-bold hover:underline">
            Terms of Service
          </Link>{" "}
          and{" "}
          <Link href="/privacy" className="text-[#0052cc] font-bold hover:underline">
            Privacy Policy
          </Link>
          .
        </p>
      ...
      {/* Page Footer */}
      <footer className="z-10 w-full bg-[#f0f4fd]/80 backdrop-blur-sm border-t border-slate-200/50 py-3.5 px-6 sm:px-12 flex flex-col sm:flex-row justify-between items-center text-[11px] text-slate-500 gap-2">
        <div>
          <span className="font-bold text-slate-900">Fintech Connect</span>{" "}
          <span>© 2024 Fintech Connect. Institutional-grade infrastructure.</span>
        </div>
